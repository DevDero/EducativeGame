...
function _CreateUserWithEmailAndPassword(email, password, objectName, callback, fallback) {
 ...
 try {
  firebase.auth().createUserWithEmailAndPassword(parsedEmail, parsedPassword).then(function(unused) {
   globalUnityInstance.SendMessage(parsedObjectName, parsedCallback, "Success: signed up for " + parsedEmail);
...
function _GetHighScore(objectName, callback, fallback) {
 var parsedObjectName = UTF8ToString(objectName);
 var parsedCallback = UTF8ToString(callback);
 var parsedFallback = UTF8ToString(fallback);
 firebase.database().ref("/users").orderByChild("totalScore").limitToLast(50).once("value").then(function(snapshot) {
...
function _HTMLButtonCreate(objectNames) {
...
function _SetJSON(path, value, objectName, callback, fallback) {
 var parsedPath = UTF8ToString(path);
...
var asmLibraryArg = {
 ...
 "GetJSON": _GetJSON,
 "HTMLButtonCreate": _HTMLButtonCreate,
